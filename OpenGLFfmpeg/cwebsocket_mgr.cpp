...
namespace chen
{
	...
	bool cwebsocket_mgr::init(std::string ws_url )
	{
		...
		//RTC_LOG(LS_WARNING) << "ws url = " << ws_url;
		...
		if (!m_ws)
		{
			...
			//RTC_LOG(LS_INFO) << "ws connect mediasoup failed !!! ws_url = " <<  ws_url.c_str();
			return false;
		}
		if (m_ws)
		{
	...
	bool cwebsocket_mgr::startup()
	{
		...
		{
			//RTC_LOG(LS_INFO) <<  "websocket status = " << m_status.load() << ", m_stoped = " <<  m_stoped.load();
			return false;
		}
	...
	void cwebsocket_mgr::_work_thread()
	{
		...
		while (!m_stoped && m_ws->getReadyState() == wsclient::WebSocket::OPEN) 
		{
			...
			if (elapse < 10)
			{
				//std::this_thread::sleep_for(std::chrono::milliseconds(10 - elapse));
			}
		}
	 ...
	 void cwebsocket_mgr::OnMessage(const std::vector<uint8_t>& message)
	{
		 ...
		 {
			 clock_guard lock(m_recv_msg_mutex);
			 m_recv_msgs.push_back(  std::string(message.begin(), message.end()));
		 }
		// ERROR_EX_LOG("[%s]", data.c_str());
	}
